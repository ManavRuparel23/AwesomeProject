...
const ServiceProviderList = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const fetchRestaurantData = async () => {
      try {
        const categorySnapshot = await firestore().collection("categories").where("name", "==", "Food").get();
        const data = [];
        if (categorySnapshot) {
  ...
  const statusColor = currentItemData.status === "Open" ? Colors.status_open_clr : Colors.status_close_clr;
  const headerContent = (
  ...
  return (
    <BackgroundImage backgroundImage={Images.background} headerContent={headerContent}>
      <View style={styles.separator} />
      <ScrollView style={{ marginBottom: 60 }}>
        <View style={styles.details_container}>
          <View style={styles.main_container}>
            <Image source={{ uri: currentItemData.image }} style={styles.main_icon} />
            <View style={styles.name_container}>
              <Text style={styles.name_text}>{currentItemData.name}</Text>
              <View style={styles.status_category_container}>
                <View style={[styles.status_text_container, { borderColor: statusColor }]}>
                  <Text style={[styles.status_text, { color: statusColor }]}>{currentItemData.status}</Text>
                </View>
          ...
          <View style={[styles.detail_fields_container, { marginRight: 18 }]}>
            ...
            <Text style={styles.detail_fields_text}>{currentItemData.address}</Text>
          </View>
          <TouchableOpacity style={styles.detail_fields_container} onPress={() => {openURL(currentItemData.website)}}>
            <View style={styles.detail_fields_icon_container}>
              <Image source={Images.website} style={styles.detail_fields_icon} />
            </View>
            <Text style={styles.detail_fields_text}>{currentItemData.website}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.detail_fields_container} onPress={handleEmailPress}>
            <View style={styles.detail_fields_icon_container}>
              <Image source={Images.mail} style={styles.detail_fields_icon} />
            </View>
            <Text style={styles.detail_fields_text}>{currentItemData.mail}</Text>
          </TouchableOpacity>
          <TouchableOpacity
              ...
              <Text style={styles.detail_fields_text}>{formatPhoneNumber(currentItemData.mobile)}</Text>
          </TouchableOpacity>
          <View style={styles.detail_fields_container}>
            <View style={styles.detail_fields_icon_container}>
              <Image source={Images.location_name} style={styles.detail_fields_icon} />
            </View>
            <Text style={styles.detail_fields_text}>{currentItemData.location_name}</Text>
          </View>
        </View>
        ...
        <View>
          <FlatList
            data={restaurantData}
            renderItem={({ item }) => renderItem({ item, navigation, setItemData: setCurrentItemData })}
